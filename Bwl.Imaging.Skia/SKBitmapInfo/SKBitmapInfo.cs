...
/// </summary>
public class SKBitmapInfo : IDisposable, ICloneable
{
    ...
    /// </summary>
    public static SKBitmap SKBitmapCloneManaged(SKBitmap bmp)
    {
    ...
    /// </summary>
    public float SKBitmapKeepTimeS { get; set; } = 5f;
    ...
    public SKBitmapInfo(byte[] jpg)
    {
        SetJpg(jpg, -1); // Создается новая сущность, без ожидания
    }

    public SKBitmapInfo(SKBitmap bmp)
    {
    ...
    public bool CompareJpgFast(SKBitmapInfo obj)
    {
        if (obj is null)
            return false;
    ...
    /// </summary>
    /// <param name="timeoutMs">Таймаут блокировки доступа к разделяемому ресурсу.</param>
    public SKBitmapInfo GetClonedCopy(int timeoutMs = 10000)
    {
        SKBitmapInfo result = null;
        BmpLock(timeoutMs);
        try
        {
            if (_jpg is not null)
            {
                result = new SKBitmapInfo(ArrayCopy(_jpg));
            }
            else if (_bmp is not null)
            {
                result = new SKBitmapInfo(BmpCloneInternal(_bmp));
            }
            else
            {
                result = new SKBitmapInfo();
            }
        }
        catch (Exception ex)
        {
    ...
    private SKBitmap GetBitmapDecodedInternal()
    {
        if (_bmp is null && _jpg is not null)
        {
            ...
            {
                SetBmpInternal(bmp);
                SKBitmapDisposeWithDelay(bmp, SKBitmapKeepTimeS); // Отложенная очистка битмапа (именного этого, контроль по ссылке)
            }
        }
        return _bmp;
    }
    ...
    private SKBitmap BmpCloneInternal(SKBitmap bmp)
    {
        ...
        try
        {
            result = SKBitmapCloneManaged(bmp);
        }
        catch (Exception ex)
        {
            Interlocked.Increment(ref _cloneErrorCount);
    ...
    private void SKBitmapDisposeWithDelay(SKBitmap bmp, float bitmapKeepTimeS)
    {
        if (bmp is not null && bitmapKeepTimeS >= 0f)
        {
            ResourceCollector<SKBitmapInfo>.Add(new CollectedSKBitmap(bmp, bitmapKeepTimeS, _bmpSemaphore, DisposeBmpInternal));
        }
    }
...
internal class CollectedSKBitmap : ICollectedResource
{
    ...
    public CollectedSKBitmap(SKBitmap bmp, float bitmapKeepTimeS, Semaphore sema, Action<SKBitmap> disposeAction)
    {
        _bmp = bmp;
        _sema = sema;
    ...
    {
        if (!SKBitmapInfo.CheckBitmap(_bmp))
        {

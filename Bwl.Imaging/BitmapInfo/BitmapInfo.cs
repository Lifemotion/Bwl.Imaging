...
namespace Bwl.Imaging
{
    ...
    public class BitmapInfo : IDisposable, ICloneable
    {
        ...
        /// </summary>
        private static bool _safeCloneMode = false;
        public static bool SafeCloneMode
        {
            get
            {
        ...
        /// </summary>
        public static Bitmap BitmapCloneManaged(Bitmap bmp)
        {
            Bitmap result = null;
            bool safeCloneMode = _safeCloneMode;
            if (bmp is not null)
            {
                try
                {
                    var clonedBmp = safeCloneMode ? new Bitmap(bmp) : UnsafeFunctions.BitmapClone(bmp);
        ...
        /// </summary>
        public static bool CheckBitmap(Bitmap bmp)
        {
            bool result;
            try
            {
                result = bmp is not null && bmp.PixelFormat != PixelFormat.DontCare && bmp.Width * bmp.Height > 0;
            }
            catch
            {
                result = false;
        ...
        /// </summary>
        public PixelFormat BmpPixelFormat
        {
            get
            {
                var item = _bmpPixelFormat;
                if (item is not null)
                {
                    return item.Value;
                }
        ...
        public bool CompareJpgFast(BitmapInfo obj)
        {
            ...
            if (jpgMe.Length != jpgObj.Length) // ...их длины не равны
            {
                return false;
            }
            else
        ...
        public void SetJpg(byte[] jpg, int timeoutMs = 10000)
        {
            ...
            if (jpgSize.Width * jpgSize.Height > 0 && new[] { 1, 3 }.Contains(jpgChannelCount))
            {
                if (timeoutMs >= 0)
                {
                    BmpLock(timeoutMs);
                ...
                try
                {
                    ...
                    _bmpSize = jpgSize;
                    _bmpPixelFormat = jpgChannelCount == 3 ? PixelFormat.Format24bppRgb : PixelFormat.Format8bppIndexed;
                    _jpg = jpg;
                }
                catch (Exception ex)
                {
                    _bmpSize = default;
                    _bmpPixelFormat = default;
                    _jpg = null;
                    throw ex;
                }
                finally
        ...
        public void SetBmp(Bitmap bmp, int timeoutMs = 10000)
        {
            if (timeoutMs >= 0)
            {
                BmpLock(timeoutMs);
        ...
        /// </summary>
        /// <param name="timeoutMs">Таймаут блокировки доступа к разделяемому ресурсу.</param>
        public Bitmap GetClonedBmp(int timeoutMs = 10000)
        {
            Bitmap result = null;
            BmpLock(timeoutMs);
            try
        ...
        /// </summary>
        /// <param name="timeoutMs">Таймаут блокировки доступа к разделяемому ресурсу.</param>
        public Bitmap GetClonedBmpGray(int timeoutMs = 10000)
        {
            Bitmap result = null;
            BmpLock(timeoutMs);
            try
        ...
        /// Установка Bitmap-а.
        /// </summary>
        private void SetBmpInternal(Bitmap bmp)
        {
            if (CheckBitmap(bmp))
            {
                _bmp = bmp;
        ...
        /// </summary>
        private Bitmap GetBitmapDecodedInternal()
        {
        ...
        /// </summary>
        private Bitmap BmpCloneInternal(Bitmap bmp)
        {
            Bitmap result = null;
            try
            {
        ...
        private Bitmap DecodeJpegInternal(byte[] jpg)
        {
            Bitmap result = null;
            try
            {
                ...
                else
                {
                    try
                    {
                        var bmp = new Bitmap(new System.IO.MemoryStream(jpg));
                        if (CheckBitmap(bmp))
                        {
                            result = bmp; // Bitmap прошел проверку
                            Interlocked.Add(ref _globalAllocatedDataCount, GetBitmapAllocatedSize(result.Size, result.PixelFormat));
        ...
        /// </summary>
        private Size GetJpegSize(byte[] jpg, ref int channelCount, int maxBytesSearch = 1024)
        {
        ...
        private void BitmapDisposeWithDelay(Bitmap bmp, float bitmapKeepTimeS)
        {
        ...
        private void DisposeBmpInternal(Bitmap target)
        {
            if (CheckBitmap(target))
            {
                try
                {
                    if (ReferenceEquals(_bmp, target))
                        _bmp = null;
                    long bitmapAllocatedSize = GetBitmapAllocatedSize(target.Size, target.PixelFormat);
                    target.Dispose();
                    Interlocked.Add(ref _globalAllocatedDataCount, -1L * bitmapAllocatedSize);
        ...
        /// </summary>
        private long GetBitmapAllocatedSize(Size bmpSize, PixelFormat bmpPixelFormat)
        {
            int bytesPerPixel = 1;
            switch (bmpPixelFormat)
            {
                ...
                default:
                    {
                        throw new Exception($"BitmapInfo.GetBitmapAllocatedSize(): Unsupported pixel format {Enum.GetName(typeof(PixelFormat), Bmp.PixelFormat)}");
                    }
            }
            int stride = 4 * ((bmpSize.Width * bytesPerPixel + 3) / 4); // 4 - DWORD
    ...
    internal class CollectedBitmap : ICollectedResource
    {
        ...
        private Action<Bitmap> _disposeAction;

        public DateTime CollectTimeUtc { get; private set; }

        public CollectedBitmap(Bitmap bmp, float bitmapKeepTimeS, Semaphore sema, Action<Bitmap> disposeAction)
        {
            _bmp = bmp;
            _sema = sema;
        ...
        public bool TryCollect()
        {
            if (!BitmapInfo.CheckBitmap(_bmp))
                return true; // Disposed в другом коде?
            if (_sema.WaitOne(0))
            {
                try
                {

...
namespace Bwl.Imaging
{
    ...
    public class RegionWithVector : Region, ICloneable
    {
        ...
        public PointF Vector { get; set; } = new PointF();

        public Vector ImagingVector
        {
            get
            {
                var point1 = new PointF(Points.Average(item => item.X), Points.Average(item => item.Y));
        ...
        public RegionWithVector() : base(Guid.NewGuid().ToString(), new[] { new PointF(), new PointF(), new PointF(), new PointF() }) // Как Tetragon
        {
        }

        public RegionWithVector(PointF[] points) : base(Guid.NewGuid().ToString(), points)
        {
        }

        public RegionWithVector(string id) : base(id, new[] { new PointF(), new PointF(), new PointF(), new PointF() }) // Как Tetragon
        {
        }

        public RegionWithVector(string id, PointF[] points) : base(id, points) // Как Tetragon
        {
        }
        ...
        public new RegionWithVector CloneWithNewPoints(PointF[] points)
        {
